...
var CONFIG_PATH =
  argv.config || Path.resolve(OS.homedir(), Constants.DEFAULT_CONFIG_FILE);
...
if (argv.verbose) {
  ...
  console.log('process.env.PKGLINK_NODE_OPTIONS', envNodeOptions);
  console.log('config', parsedConfigJson);
  console.log('freeMemoryMB', freeMemoryMB);
}

var noOverrideNotEnoughMemory =
  !argv.memory && !envNodeOptions && !configMemory && !hasExtraMemory;
...
if (noOverrideNotEnoughMemory || alreadyHasOptions) {
  if (!alreadyHasOptions) {
    // indicate that we are running as is
    console.log(
      'running with reduced memory, free:%sMB desired:%sMB',
      freeMemoryMB,
      DESIRED_MEM
    );
  }
  ...
  // need to use child to get right options
  if (argv.verbose) {
